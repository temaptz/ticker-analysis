...
def generate_data():
    ...
    print(f'GENERATE DATA {learn.model.TA_3_volume}')
    ...
    for instrument in instruments_list:
        ...
        for date in date_utils.get_dates_interval_list(
            ...
            print(f'(TA-3 VOLUME PREPARE: {counter_total}; ERROR: {counter_error}; CACHED: {counter_cached}; ADDED: {counter_added}; redis: {redis_utils.get_redis_size_mb()}MB/{redis_utils.get_redis_max_size_mb()}MB; CURRENT_TICKER: {instrument.ticker}({instrument_index}/{len(instruments_list)}))')
...
def get_model_file_path():
    if docker.is_docker():
        return '/app/learn_models/ta-3_volume.cbm'

    return utils.get_file_abspath_recursive('ta-3_volume.cbm', 'learn_models')


def get_data_frame_csv_file_path():
    if docker.is_docker():
        return '/app/ta-3_volume.csv'

    return utils.get_file_abspath_recursive('ta-3_volume.csv', 'data_frames')


def cache_record(card: learn.ta_3_volume.Ta3VolumeAnalysisCard) -> None:
