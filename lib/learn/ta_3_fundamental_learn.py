...
def generate_data():
    ...
    for instrument in instruments_list:
        ...
        for date in date_utils.get_dates_interval_list(date_from=date_start, date_to=date_end, interval_seconds=(3600 * 24 * 7), is_skip_holidays=False, is_order_descending=not docker.is_docker()):
            ...
            print(f'(TA-3 FUNDAMENTAL PREPARE: {counter_total}; ERROR: {counter_error}; CACHED: {counter_cached}; ADDED: {counter_added}; redis: {redis_utils.get_redis_size_mb()}MB/{redis_utils.get_redis_max_size_mb()}MB; CURRENT_TICKER: {instrument.ticker}({instrument_index}/{len(instruments_list)}))')
...
def get_model_file_path():
    if docker.is_docker():
        return '/app/learn_models/ta-3_fundamental.cbm'

    return utils.get_file_abspath_recursive('ta-3_fundamental.cbm', 'learn_models')


def get_data_frame_csv_file_path():
    if docker.is_docker():
        return '/app/ta-3_fundamental.csv'

    return utils.get_file_abspath_recursive('ta-3_fundamental.csv', 'data_frames')


def cache_record(card: learn.ta_3_fundamental.Ta3FundamentalAnalysisCard) -> None:
