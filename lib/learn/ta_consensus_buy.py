...
MODEL_NAME = learn.model.TA_consensus_buy
...
class TaConsensusBuyCard:
    ...
    def __init__(
        ...
        try:
            ...
            print('ERROR INIT TaConsensusBuyCard', e)
            self.is_ok = False

    def fill_card(self):
        self.price = instruments.get_instrument_price_by_date(uid=self.instrument.uid, date=self.date)
        self.macd_rated = agent.macd.macd_buy_rate(instrument_uid=self.instrument.uid, date=self.date)
        self.rsi_rated = agent.rsi.rsi_buy_rate(instrument_uid=self.instrument.uid, date=self.date)
        self.tech_rated = agent.tech.get_tech_buy_rate(instrument_uid=self.instrument.uid, date=self.date)
        self.news_rated = agent.news.get_news_buy_rate(instrument_uid=self.instrument.uid, date=self.date)
        self.fundamental_rated = agent.fundamental.get_fundamental_buy_rate(instrument_uid=self.instrument.uid, date=self.date)
        self.volume_rated = agent.volume.get_volume_buy_rate(instrument_uid=self.instrument.uid, date=self.date)
        self.profit_rated = agent.profit.get_profit_buy_rate(instrument_uid=self.instrument.uid, account_id=self.account_id)
        ...
        if (tech := tech_analysis.get_tech_analysis(
            ...
            if max_sma_profit := max([utils.get_price_by_quotation(i.signal) for i in tech]):
                if (max_price_change := utils.get_change_relative(
                    ...
                    return agent.utils.linear_interpolation(max_price_change, 0, 0.15, 0, 1)
