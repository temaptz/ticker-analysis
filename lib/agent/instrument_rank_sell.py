...
def llm_price_prediction_rate(state: State):
    if instrument_uid := state.get('instrument_uid', None):
        ...
        2. Учитывай что акции выгоднее продавать при высокой цене.
        3. Прогноз цены на длительный период может быть менее точным.
        4. Если в будущем согласно прогнозу цена будет расти, то продажа сейчас менее выгодна.
        5. Оцени, насколько выгодна продажа акций именно сейчас.
        4. Присвой итоговую числовую оценку от 0 до 100, где:
           - 0-25 - высокий потенциал роста, продажа сейчас не выгодна;
           - 26-50 - нестабильно высокий потенциал роста, сейчас продажа не выгодна;
           - 51-75 - незначительное колебание потенциала около текущей цены, продажа сейчас возможна;
           - 76-100 - рост стабильно отрицательный, сейчас оптимальный момент для продажи.
...
def llm_total_sell_rate(state: State):
    if instrument_uid := state.get('instrument_uid', None):
        ...
        if invest_rate or price_prediction_rate:
            ...
            Комментарий оценки потенциальной выгоды:
            
            {invest_rate_conclusion or 'Unknown'}
            
            Комментарий оценки оптимального момента продажи:
            ...
            3. Присвой итоговую оценку от 0 до 100, где:
               - 0-25 - продажа сейчас не выгодна;
               - 26-50 - продажа сейчас умеренно выгодна, момент продажи не удачный;
               - 51-74 - продажа сейчас выгодна, момент для продажи средне удачный.
               - 75-100 - максимально выгодный и удачный момент для продажи.
