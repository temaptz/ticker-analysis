...
def instrument_recommendation_create(state: State) -> State:
    ...
    if uid:
        ...
        2. Подбери оптимальную выгодную стоимость покупки инструмента при создании торговой заявки с учетом дневного колебания курса. Стоимость покупки может быть чуть ниже текущей стоимости. Стоимость покупки должна быть на 2.5% ниже текущей стоимости.
        3. Подбери количество единиц инструмента, которое можно купить при создании торговой заявки.
        4. Общая стоимость покупки(количество*цена) должна быть меньше 25% от всех доступных средств.            
